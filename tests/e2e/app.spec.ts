...
async function connect(page: Page, remember = false) {
  ...
  await page.getByLabel('在此设备记住授权', { exact: true }).setChecked(remember);
...
test('remembered authorization survives reopening, stays out of exports and clears across tabs', async ({
  page,
  context,
}) => {
  const remote = new MockGitHub();
  ...
  await page.close();
  await reopened.getByRole('button', { name: '同步', exact: true }).click();
  await expect(reopened.getByRole('button', { name: '同步', exact: true })).toBeEnabled();
  await expect(reopened.getByRole('dialog', { name: '连接 GitHub 仓库' })).toBeHidden();
  await reopened.getByRole('button', { name: '设置与本地数据', exact: true }).click();
  await expect(reopened.getByLabel('在此设备记住授权', { exact: true })).toBeChecked();
  await reopened.screenshot({
    path: 'artifacts/inkbridge-remember-authorization.png',
    fullPage: true,
  });
  const download = reopened.waitForEvent('download');
  await reopened.getByRole('button', { name: '导出 ZIP', exact: true }).click();
  ...
  expect(await reopened.evaluate(() => JSON.stringify(localStorage))).not.toContain(
    'github_pat_simulated_only',
  );
  const second = await context.newPage();
  await start(second);
  await second.getByRole('button', { name: '设置与本地数据', exact: true }).click();
  await expect(second.getByLabel('在此设备记住授权', { exact: true })).toBeChecked();
  await reopened.getByRole('button', { name: '清除授权', exact: true }).click();
  await expect(reopened.getByLabel('在此设备记住授权', { exact: true })).not.toBeChecked();
  await expect(second.getByLabel('在此设备记住授权', { exact: true })).not.toBeChecked();
  await second.getByRole('button', { name: '关闭设置' }).click();
  await second.getByRole('button', { name: '同步', exact: true }).click();
  await expect(second.getByRole('dialog', { name: '连接 GitHub 仓库' })).toBeVisible();
  await reopened.reload();
  await expect(reopened.getByRole('textbox', { name: '编辑笔记' })).toContainText('原始内容');
  await reopened.getByRole('button', { name: '同步', exact: true }).click();
  await expect(reopened.getByRole('dialog', { name: '连接 GitHub 仓库' })).toBeVisible();
  await second.close();
  await reopened.close();
});

test('an existing session can be remembered in settings and forgetting keeps only that session', async ({
  page,
  context,
}) => {
  const remote = new MockGitHub();
  ...
  await start(page);
  await connect(page);
  const settings = page.getByRole('button', { name: '设置与本地数据', exact: true });
  ...
  await expect(page.getByRole('dialog', { name: '连接 GitHub 仓库' })).toBeHidden();
  await page.reload();
  await page.getByRole('textbox', { name: '编辑笔记' }).waitFor();
  await page.getByRole('button', { name: '同步', exact: true }).click();
  await expect(page.getByRole('dialog', { name: '连接 GitHub 仓库' })).toBeVisible();
});
...
test('manual sync saves fresh input, shows progress and sends one commit per click', async ({
  ...
  await expect(sync).toHaveText('同步');
  let release!: () => void;
  let entered = false;
